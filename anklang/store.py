...
class ProblemStore:
    ...
    def _migrate_legacy_columns(self) -> None:
        ...
        self._conn.execute(
            "DELETE FROM index_metadata WHERE key NOT IN ('embedding_model', 'embedding_dimensions')"
        )
    ...
    def _metadata_locked(self) -> IndexMetadata | None:
        ...
        return IndexMetadata(model, parsed_dimensions)
        ...
        with self._lock:
            ...
            try:
                ...
                if metadata is None:
                    ...
                    metadata = IndexMetadata(spec.model, spec.dimensions)
                else:
