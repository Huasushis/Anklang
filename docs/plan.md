...
- 版本：0.1（历史规划稿，仓库已有可运行实现）
- 日期：2026-07-26

> 当前状态（2026-07-31）：仓库已经使用 Python 3.11 标准库完成服务、接口校验、转发检索、
> 本地检索框架和来源导入框架，不依赖 FastAPI、Pydantic 或其他第三方包。本文件保留最初的取舍、
> 风险和分阶段安排，当前运行方式与环境变量以根目录 `README.md` 和 `.env.example` 为准。
...
- 整个请求受 Urmotiv 侧配置的 `timeoutMs` 限制（默认 120000 毫秒，范围 1000-120000）。超时后
...
- `sources/<name>/` 每个目录对应一个题目来源。当前实现使用
  `fetch_new_problems(since: str | None) -> list[RawProblem]`；`RawProblem` 包含题号、标题、
  题面、可选链接、题目更新时间和可选原始记录引用。
- `since` 与题目更新时间若非空，统一使用带毫秒、以 `Z` 结尾的 UTC 时间。最初“来源可以把任意
  分页编号当公共游标”的设想已取消：任意字符串无法可靠比较先后，会让旧任务把游标倒退。来源自己
  的分页编号可以保存在该来源的私有状态中，不与公共时间游标混用。
- 没有稳定题目更新时间的来源仍可首次入库；同一题号之后出现不同内容时保守拒绝覆盖，不按到达
  顺序猜测新旧。批内重复题号只保留明确较新的版本，同一时间却内容冲突时整组跳过。
